...
// ----------------------------------------------------------------------------
//  ---------------------------------------------------------------------------
class stex extends Exchange["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    'Wrong parameters': errors.BadRequest,
                    ...
                    'Too Many Attempts.': errors.DDoSProtection,
                    'Selected Pair is disabled': errors.BadSymbol,
                    'Invalid scope(s) provided.': errors.PermissionDenied,
                    'The maximum amount of open orders with the same price cannot exceed 10': errors.InvalidOrder,
                    'Your account not verified!': errors.AccountSuspended, // {"success":false,"message":"Your account not verified!","unified_message":{"message_id":"verification_required_to_continue","substitutions":null},"notice":"Please be informed that parameter `message` is deprecated and will be removed. Use unified_message instead."}
                },
                'broad': {
                    'Not enough': errors.InsufficientFunds, // {"success":false,"message":"Not enough  ETH"}
                },
            },
        });
    }
    ...
    async fetchOHLCV(symbol, timeframe = '1m', since = undefined, limit = undefined, params = {}) {
        ...
        const request = {
            'currencyPairId': market['id'],
            'candlesType': this.safeString(this.timeframes, timeframe, timeframe), // default 1d
    ...
    parseOrder(order, market = undefined) {
        ...
        if (filled !== undefined) {
            if (amount !== undefined) {
                ...
                remaining = Precise["default"].stringMax(remaining, '0.0');
            }
            if (price !== undefined) {
                if (cost === undefined) {
    ...
    async cancelOrder(id, symbol = undefined, params = {}) {
        ...
        if (numAcceptedOrders < 1) {
            if (numRejectedOrders < 1) {
                throw new errors.OrderNotFound(this.id + ' cancelOrder() received an empty response: ' + this.json(response));
            }
            else {
    ...
    async fetchDepositAddress(code, params = {}) {
        ...
        if (wallet === undefined) {
            throw new errors.ExchangeError(this.id + ' fetchDepositAddress() could not find the wallet id for currency code ' + code + ', try to call createDepositAddress() first');
        }
    ...
    async transfer(code, amount, fromAccount, toAccount, params = {}) {
        ...
        if (fromAccount === 'referal' && toAccount === 'spot') {
            ...
            throw new errors.ExchangeError(this.id + ' transfer() only allows transfers of referal to spot and between a walletId and funding');
        }
        const response = await this[method](this.extend(request, params));
        //
        ...
        if (fillResponseFromRequest) {
            ...
            if (typeof amount === 'string' && Precise["default"].stringLt(amount, '0')) {
                amount = this.parseNumber(Precise["default"].stringNeg(amount));
            }
