...
class zb extends zb$1 {
    ...
    async watchOHLCV(symbol, timeframe = '1m', since = undefined, limit = undefined, params = {}) {
        ...
        const market = this.market(symbol);
        if (market['spot']) {
            throw new errors.NotSupported(this.id + ' watchOHLCV() supports contract markets only');
        }
    ...
    handleTrades(client, message, subscription) {
        ...
        if (array === undefined) {
            ...
            array = new Cache.ArrayCache(limit);
        }
    ...
    async watchOrderBook(symbol, limit = undefined, params = {}) {
        if (limit !== undefined) {
            if ((limit !== 5) && (limit !== 10)) {
                throw new errors.ExchangeError(this.id + ' watchOrderBook limit argument must be undefined, 5, or 10');
            }
        }
        else {
            limit = 5; // default
