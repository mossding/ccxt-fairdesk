...
class mexc extends mexc$1 {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'ws': {
                    'exact': {
                        'signature validation failed': errors.AuthenticationError, // { channel: 'sub.personal', msg: 'signature validation failed'}
                    },
                    'broad': {
                        'Contract not exists': errors.BadSymbol, // { channel: 'rs.error', data: 'Contract not exists', ts: 1651509181535}
                    },
                },
            },
        });
    ...
    async watchTicker(symbol, params = {}) {
        ...
        if (market['type'] === 'spot') {
            throw new errors.NotSupported(this.id + ' watchTicker does not support spot markets');
        }
        else {
    ...
    async watchOrderBook(symbol, limit = undefined, params = {}) {
        ...
        if (limit !== undefined) {
            if (limit !== 5 && limit !== 10 && limit !== 20) {
                throw new errors.BadRequest(this.id + ' watchOrderBook limit parameter cannot be different from 5, 10 or 20');
            }
        }
        else {
            limit = 20;
    ...
    async watchMyTrades(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        let trades = undefined;
        if (type === 'spot') {
            throw new errors.NotSupported(this.id + ' watchMyTrades does not support spot markets');
        }
        else {
    ...
    async watchBalance(params = {}) {
        ...
        if (type === 'spot') {
            throw new errors.NotSupported(this.id + ' watchBalance does not support spot markets');
        }
        else {
