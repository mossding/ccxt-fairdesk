...
class mexc extends Exchange["default"] {
    ...
    async createSpotOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (isMarketOrder) {
            throw new errors.InvalidOrder(this.id + ' createOrder () does not support market orders, only limit orders are allowed');
        }
        if (orderType === 'LIMIT') {
        ...
        if (timeInForce === 'FOK') {
            throw new errors.InvalidOrder(this.id + ' createOrder () does not support timeInForce FOK, only IOC, PO, and GTC are allowed');
        }
        if (((orderType !== 'POST_ONLY') && (orderType !== 'IMMEDIATE_OR_CANCEL') && (orderType !== 'LIMIT_ORDER'))) {
            throw new errors.InvalidOrder(this.id + ' createOrder () does not support ' + type + ' order type, only LIMIT, LIMIT_ORDER, POST_ONLY or IMMEDIATE_OR_CANCEL are allowed');
        }
        const request = {
            'symbol': market['id'],
    ...
    async createSwapOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (openType === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' createSwapOrder () requires an integer openType parameter, 1 for isolated margin, 2 for cross margin');
        }
        ...
        if ((side !== 1) && (side !== 2) && (side !== 3) && (side !== 4)) {
            throw new errors.InvalidOrder(this.id + ' createSwapOrder () order side must be 1 open long, 2 close short, 3 open short or 4 close long');
        }
        const request = {
            'symbol': market['id'],
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        ...
        if (firstOrder === undefined) {
            throw new errors.OrderNotFound(this.id + ' fetchOrder() could not find the order id ' + id);
        }
