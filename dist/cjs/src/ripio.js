...
class ripio extends Exchange["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {},
                'broad': {
                    'Authentication credentials were not provided': errors.AuthenticationError,
                    'Disabled pair': errors.BadSymbol,
                    'Invalid order type': errors.InvalidOrder,
                    'Your balance is not enough': errors.InsufficientFunds,
                    "Order couldn't be created": errors.ExchangeError,
                    ...
                    'Invalid pair': errors.BadSymbol,
                    'amount must be a number': errors.BadRequest,
                    'Total must be at least': errors.InvalidOrder,
                    'Account not found': errors.BadRequest,
                    'Wrong password provided': errors.AuthenticationError,
                    'User tokens limit': errors.DDoSProtection,
                    'Something unexpected ocurred': errors.ExchangeError,
                    ...
                    'account_balance': errors.BadRequest,
                },
            },
        });
    }
    ...
    handleErrors(code, reason, url, method, headers, body, response, requestHeaders, requestBody) {
        ...
        if (errors$1 !== undefined) {
            const feedback = this.id + ' ' + body;
            const keys = Object.keys(errors$1);
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                const error = this.safeValue(errors$1, key, []);
