...
class bkex extends Exchange["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    '1005': errors.InsufficientFunds,
                },
                'broad': {
                    'Not Enough balance': errors.InsufficientFunds,
                    'Order does not exist': errors.InvalidOrder,
                    'System busy, please try again later': errors.BadRequest, // in my tests, this was thrown mostly when request was bad, not the problem of exchange. It is easily reproduced in 'cancelOrders'
                },
            },
        });
    }
    ...
    async fetchTickers(symbols = undefined, params = {}) {
        ...
        if (symbols !== undefined) {
            if (!Array.isArray(symbols)) {
                throw new errors.BadRequest(this.id + ' fetchTickers() symbols argument should be an array');
            }
        }
        ...
        if (symbols !== undefined) {
            ...
            if (market['swap']) {
                if (Array.isArray(symbols)) {
                    ...
                    if (symbolsLength > 1) {
                        throw new errors.BadRequest(this.id + ' fetchTickers() symbols argument cannot contain more than 1 symbol for swap markets');
                    }
                }
                request['symbol'] = market['id'];
            }
